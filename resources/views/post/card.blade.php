<!-- ====== Cards Section Start -->
<div class="py-7 border-b-primary border-b-2 mb-14 drop-shadow-lg
    ...
    <div class="controls w-full text-right px-8 mb-4">
      <x-button-link href="{{ route('post.edit',$post) }}" ><i class="fa-sharp fa-solid fa-pen-to-square mr-2"></i>Modify</x-button-link>
      <x-form.form id="delete_post" route="{{ route('post.delete', $post) }}" method="delete" class="inline-block ml-5">
        <x-form.button variant="red"><i class="fa-solid fa-trash-can mr-2"></i>Delete</x-form.button>
      </x-form.form>
      {{-- <x-button-link href="{{ route('post.delete', $post) }}" variant="red" ><i class="fa-solid fa-trash-can mr-2"></i>Delete</x-button-link> --}}
    </div>
</div>
