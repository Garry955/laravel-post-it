<a href="{{ $href }}" target="{{ $target = '_self' }}" {{ $attributes->class([
    ...
    'border-red w-full rounded border-red-500 border-2 py-1 ml-5 px-5 text-red-500 hover:text-white hover:scale-105 hover:bg-red-500' => $variant === 'red',
    'border-primary w-full rounded py-1 ml-5 px-5 border-2 border-primary text-primary hover:text-white hover:bg-primary hover:scale-105' => $variant === 'primary'
